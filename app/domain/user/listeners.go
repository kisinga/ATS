...
type Listeners struct {
	...
	// map[int]primitive.ObjectID            map[primitive.ObjectID]chan<- *models.User
	// this would allow us to split read and write operations but introduce complexity for update operations
	// type ListenerList map[int]map[primitive.ObjectID]chan<- *models.User

	list map[TopicNames]map[primitive.ObjectID]chan<- *models.User
	mu   sync.RWMutex
}

// NewListeners returns an instance of active listeners
func NewListeners() *Listeners {
	list := make(map[int]map[primitive.ObjectID]chan<- *models.User)
	for i := 0; i < LastEffectName; i++ {
		list[i] = make(map[primitive.ObjectID]chan<- *models.User)
	}
	return &Listeners{
		list: make(map[TopicNames]map[primitive.ObjectID]chan<- *models.User),
		mu:   sync.RWMutex{},
	}
}

// AddListener appends the provided value to the list of listeners
func (l *Listeners) AddListener(ctx context.Context, channel chan<- *models.User, effectName TopicNames) primitive.ObjectID {
